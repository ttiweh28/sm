...
int main(void)
{
    ...
    fgets(string,1000, stdin);  //get the string from the terminal (a maximum of 1000)
    printf("\n Enter the position> ");
    scanf(" %i",&position);

    if((result = remove_aCharacter(string, position)) != NULL){
        printf(" The string %s  with char at %i position removed is %s",string, position, result);
    }
    else{
